...
public class HK(IYfinancePriceCrawler crawler) : BaseExchange, IToAastockStockId, IToYahooStockId, IGetPriceCrawler
{
    ...
    {
        return Convert.ToInt32(ConvertToStockIdRaw(stockId)).ToString("00000");
    }
    
    public string ToYahooStockId(string stockId)
    {
        return Convert.ToInt32(ConvertToStockIdRaw(stockId)).ToString("0000") + "." + MarketId;
    }
