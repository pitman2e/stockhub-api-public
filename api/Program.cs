...
builder.Services.Configure<ApiBehaviorOptions>(options =>
{
    options.InvalidModelStateResponseFactory = context =>
    {
        var errors = context.ModelState
            ...
            .Select(e => new
            {
                // Convert ModelState key to camelCase
                Field = !string.IsNullOrEmpty(e.Key) 
                    ? JsonNamingPolicy.CamelCase.ConvertName(e.Key) 
                    : e.Key,
...
if (app.Environment.IsDevelopment())
{
    ...
    {
        logger.LogInformation("DEMO Mode: Update Stock Pos");
        using var scope = app.Services.CreateScope();
        var positionValueService = scope.ServiceProvider.GetRequiredService<PositionValueService>();
        await positionValueService.UpdateStockPositionAsync(new UPSFilter(true, true, true));
    }
}
