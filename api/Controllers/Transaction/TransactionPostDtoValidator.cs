        ...
        RuleFor(x => x)
            .CustomAsync(async (dto, validationContext, cancellation) =>
            {
                ...
                if (stock != null)
                {
                    ...
                    {
                        validationContext.AddFailure(JsonNamingPolicy.CamelCase.ConvertName(nameof(dto.TranType)), 
                            "Transaction Type must be CASH");
                    }
                }
                ...
                if (stock != null && portfolio != null && dto.TranType == StockTransaction.TRANTYPE_SELL)
                {
                    ...
                    {
                        validationContext.AddFailure(JsonNamingPolicy.CamelCase.ConvertName(nameof(dto.TxCount)), 
                            $"Cannot sell more than Open Pos Qty {openPosCnt}");
                    }
                }
            });
    }
